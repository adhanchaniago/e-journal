...
class Teacher extends CI_Controller
{
    ...
    function journal()
    {
        ...
        $page_data['journal']  = $this->db->get_where('subject', array('teacher_id' => $teacher_id))->result_array();
        $this->load->view('backend/index', $page_data);
    }

    function journal_view($subject_id)
    {
        if ($this->session->userdata('teacher_login') !=1)
            redirect(base_url(), 'refresh');
        
        $teacher_id = $this->session->userdata('teacher_id');
        $page_data['page_name']  = 'journal_view';
        $page_data['page_title'] = get_phrase('journal_view');
        $page_data['subject']  = $this->db->get_where('subject', array('subject_id' => $subject_id))->row();
        $page_data['class_id'] = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->class_id;
        $page_data['section_id'] = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->section_id;
        $page_data['subject_id'] = $subject_id;
        $where = array(
            'subject_id' => $subject_id,
            'teacher_id' => $teacher_id
        );
        $page_data['journal'] = $this->db->get_where('journal', $where)->result_array();

        $this->load->view('backend/index', $page_data);
    }

    function journal_add($subject_id)
    {
        if ($this->session->userdata('teacher_login') !=1)
            redirect(base_url(), 'refresh');
        
        $teacher_id = $this->session->userdata('teacher_id');
        $page_data['page_name']  = 'journal_add';
        $page_data['page_title'] = get_phrase('journal_view');
        $page_data['subject']  = $this->db->get_where('subject', array('subject_id' => $subject_id))->row();
        $page_data['class_id'] = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->class_id;
        $page_data['section_id'] = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->section_id;
    ...
    function journal_edit($journal_id)
    {
        if ($this->session->userdata('teacher_login') !=1)
            redirect(base_url(), 'refresh');
        
        $teacher_id = $this->session->userdata('teacher_id');
        $page_data['page_name']  = 'journal_edit';
        $page_data['page_title'] = get_phrase('journal_view');
        $subject_id = $this->db->get_where('journal', array('journal_id' => $journal_id))->row()->subject_id;
        $page_data['subject'] = $this->db->get_where('subject', array('subject_id' => $subject_id))->row();
        $page_data['class_id'] = $this->db->get_where('journal', array('journal_id' => $journal_id))->row()->class_id;
        $page_data['section_id'] = $this->db->get_where('journal', array('journal_id' => $journal_id))->row()->section_id;
        $page_data['journal'] = $this->db->get_where('journal', array('journal_id' => $journal_id))->row();
        $page_data['journal_id'] = $journal_id;

        $this->load->view('backend/index', $page_data);
    }

    function journal_save($subject_id)
    {
        if ($this->session->userdata('teacher_login') !=1)
            redirect(base_url(), 'refresh');
        ...
        $data['submain_material']   = html_escape($this->input->post('submain_material'));
        $data['duration']           = $this->input->post('duration');
        $data['lesson_material']    = html_escape($this->input->post('lesson_material'));
        $data['implementation']     = $this->input->post('implementation');
        $data['reference_source']   = html_escape($this->input->post('reference_source'));
        $data['teaching_method']    = $this->input->post('teaching_method');
        $data['description']        = html_escape($this->input->post('description'));
        $data['date']               = $this->input->post('date');
        $data['time']               = $this->input->post('time');
        $data['student_present']    = $this->input->post('student_present');
        $data['student_absent']     = $this->input->post('student_absent');
        $data['file']     = $this->input->post('file');

        $this->db->insert('journal', $data);

        $this->session->set_flashdata('flash_message' , get_phrase('new_journal_added_successfully'));
        redirect(site_url('teacher/journal_view/'.$subject_id), 'refresh');
    }

    function journal_update($journal_id)
    {
        if ($this->session->userdata('teacher_login') !=1)
            redirect(base_url(), 'refresh');
